...
export const postDailyLog = async (request: Request, response: Response): Promise<void> => {
  ...
  if (!parsed.success) { response.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Invalid daily log data.', details: parsed.error.flatten() } }); return; }
  try {
  ...
  } catch (error) { if ((error as { code?: string }).code === '23505') { response.status(409).json({ error: { code: 'DAILY_LOG_ALREADY_EXISTS', message: 'A log already exists for this day.' } }); return; } throw error; }
};
export const listDailyLogs = async (request: Request, response: Response): Promise<void> => {
  ...
  if (!parsed.success) { response.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Provide valid from and to dates.' } }); return; }
