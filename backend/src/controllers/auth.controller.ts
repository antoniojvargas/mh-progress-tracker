...
export const googleCallback = async (request: Request, response: Response): Promise<void> => {
  try {
    ...
    if (typeof code !== 'string' || !statesMatch) {
      logger.warn('Rejected Google OAuth callback', { hasCode: typeof code === 'string', statesMatch });
      response.status(400).send('Invalid Google authorization response.');
      return;
    }
    ...
    response.redirect(`${env.frontendUrl}/?error=google-auth-failed`);
  }
};
...
export const logout = (request: Request, response: Response): void => {
  ...
  response.clearCookie('session', cookieOptions).status(204).send();
};
